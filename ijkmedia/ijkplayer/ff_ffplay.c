...
//      of the seek_pos/seek_rel variables
        ...
        ffp->record_error = ffp_record_file(ffp, pkt);// add by liuyi
...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        ...
        //编码器名称
        avcodec_name = avcodec_get_name(in_stream->codec->codec_id);

        AVStream *out_stream = avformat_new_stream(ffp->m_ofmt_ctx, in_stream->codec->codec);
        if (!out_stream) {
            av_log(ffp, AV_LOG_ERROR, "liuyi --->Failed allocating output stream\n");
            goto end;
        }
        ...
        if (avcodec_copy_context(out_stream->codec, in_stream->codec) < 0) {
            av_log(ffp, AV_LOG_ERROR, "liuyi ---> Failed to copy context from input to output stream codec context\n");
            goto end;
        }


        av_log(ffp, AV_LOG_DEBUG, "liuyi --->录制 %s\n" ,avcodec_name);
    ...
    ffp->record_error = 0;
    ffp->is_first = 0;
    av_log(ffp, AV_LOG_DEBUG, "liuyi --->开始录制\n");
...
int ffp_record_file(FFPlayer *ffp, AVPacket *packet)
{
    ...
    if (ffp->is_record) {
        if (packet == NULL) {
            ffp->record_error = 1;
            av_log(ffp, AV_LOG_ERROR, "liuyi ffp_record_file packet == NULL");
            return -1;
        }
        ...
        if (0 == av_packet_ref(pkt, packet)) {
            ...
            //if (pkt->pts<=max_ts){ 
            if (pkt->pts<0){ 
                av_log(ffp, AV_LOG_ERROR, "liuyi ffp_record_file max_ts=%d,pts=%d",max_ts,pkt->pts); 
              max_ts++;
              pkt->pts = max_ts; 
              pkt->dts = max_ts; 
            }else{
            ...
            // 写入一个AVPacket到输出文件
            if ((ret = av_interleaved_write_frame(ffp->m_ofmt_ctx, pkt)) < 0) { // todo 出现负数导致写包错误
                av_log(ffp, AV_LOG_ERROR, "liuyi----> faild: av_interleaved_write_frame");  
            }else{
                av_log(ffp, AV_LOG_DEBUG, "liuyi----> ok : av_interleaved_write_frame");   
            }
